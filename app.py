...
def home():
    ...
    db_response = db.session.query(Projects).filter(
        Projects.city.ilike(f'%{"Kenya"}%')
        ).all()
    #Uganda
    uganda=  db.session.query(Projects).filter(
        Projects.city.ilike(f'%{"Uganda"}%')
        ).all()
    nigeria=len(db.session.query(Projects).filter(Projects.city.ilike(f'%{"Nigeria"}%')).all())
    ...
    times=[]
    for x in countries:
        occurrences=len(db.session.query(Projects).filter(
        Projects.city.ilike(f'%{countries[x]}%')
        ).all())
        codes.append(x)
        times.append(occurrences)
    z = dict(zip(codes, times))
    print(z)
    return render_template('home.html',title="Dashboard",nigeria=nigeria,engagements=len(engagements),uganda=uganda, kenya=kenya, countries=z,user=user)
...
@app.route("/test")
def test():
    return render_template('rimui.html')
